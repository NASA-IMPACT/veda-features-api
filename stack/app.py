...
# Tag infrastructure
for key, value in {
    "Project": api_settings.name,
    "Stack": api_settings.stage,
    "Owner": api_settings.owner,
    "Client": api_settings.client,
}.items():
    if value:
...
LambdaStack(
    app,
    id=f"{api_settings.name}-{api_settings.stage}",
    name=api_settings.name,
    stage=api_settings.stage,
    env={
